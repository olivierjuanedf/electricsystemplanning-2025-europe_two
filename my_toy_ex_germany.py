# -*- coding: utf-8 -*-
"""
First very simple toy Unit Commitment (UC) model of Germany zone - alone -> with PyPSA and ERAA data
-> adapted from my_toy_ex_italy.py
"""
...
# (0.b) Setup simulation run ID for organized output (optional, can be disabled)
# Set to True to organize outputs by simulation run
USE_SIMULATION_RUN_ID = True
...
"""
# (I.1) unique country modeled in this example -> Germany
# some comments are provided below to explain how PyPSA model could be
...
uc_period_start = datetime(year=1900, month=11, day=13)
uc_period_end = uc_period_start + timedelta(days=1)

# Set simulation run ID for organized output (if enabled)
if USE_SIMULATION_RUN_ID:
    _timestamp = datetime.now().strftime('%Y%m%d_%H%M')
    _simulation_run_id = f'{year}_cy{climatic_year}_{uc_period_start.month:02d}{uc_period_start.day:02d}-{uc_period_end.month:02d}{uc_period_end.day:02d}_{_timestamp}'
    from common.long_term_uc_io import set_simulation_run_id
    set_simulation_run_id(_simulation_run_id)
    print(f'Simulation Run ID: {_simulation_run_id}')
...
"""
  IV) Build PyPSA model - with unique country (Germany here)
    ...
    toy_model_params/germany_parameters.py
...
# Germany's energy mix with data extracted from ERAA 2023-2 dataset
...
# -> from toy_model_params/germany_parameters.py script
...
# N.B. Setting toy_model_output to True is just to have the plotted figure saved in a subfolder monozone_ger
...
if optim_status == pypsa_opt_resol_status:
    ...
    # saved in file output/long_term_uc/monozone_ger/figures/prod_germany_{year}_{period start, under format %Y-%m-%d}.png
    ...
    # -> saved in file output/long_term_uc/monozone_ger/figures/prices_germany_{year}
