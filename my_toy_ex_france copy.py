# -*- coding: utf-8 -*-
"""
First very simple toy Unit Commitment (UC) model of France zone - alone -> with PyPSA and ERAA data
...
# (III.1) Get data for france... just for test -> data used when writing PyPSA model will be re-obtained afterwards
...
"""
  IV) Build PyPSA model - with unique country (france here)
    (1) Initialize a network, the key component in PyPSA
    (2) Define specific parameters of your country to this network model 
    KEY POINT: main parameters needed for france description in PyPSA are set in script 
    toy_model_params/france_parameters.py
...
# (IV.2) Define france parameters
# (IV.2.i) Add bus for considered country
# N.B. france coordinates set randomly! (not useful in the calculation that will be done this week)
...
# Almost 'real france'... excepting hydraulic storage and Demand-Side Response capacity 
...
# -> from toy_model_params/france_parameters.py script
...
if optim_status == pypsa_opt_resol_status:
    ...
    # saved in file output/long_term_uc/monozone_ita/figures/prod_france_{year}_{period start, under format %Y-%m-%d}.png
    ...
    # -> saved in file output/long_term_uc/monozone_ita/figures/prices_france_{year}
